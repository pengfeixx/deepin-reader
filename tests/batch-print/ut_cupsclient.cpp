...
public:
    ...
    bool submitJobResult = true;
    int submitJobCallCount = 0;
    QString lastTitle;
    QString lastPrinter;
    QString lastFilename;
    int lastNumOptions = -1;
    QString lastSubmitPath;
    QString lastSubmitTitle;
    PrintSettings lastSubmitSettings;
    ...
    bool submitJob(const QString &pdfPath, const QString &title,
                   const PrintSettings &settings) override {
        lastSubmitPath = pdfPath;
        lastSubmitTitle = title;
        lastSubmitSettings = settings;
        ++submitJobCallCount;
...
// Verify that submitJob propagates title and print settings to the backend
TEST_F(CupsClientTest, MockSubmitJobSettingsPropagation) {
    MockCupsApi mock;
    PrintSettings settings;
    settings.copies = 3;
    ...
    EXPECT_TRUE(mock.submitJob(QStringLiteral("/tmp/output.pdf"),
                               QStringLiteral("doc.pdf"), settings));
    EXPECT_EQ(mock.submitJobCallCount, 1);
    EXPECT_EQ(mock.lastSubmitPath, QStringLiteral("/tmp/output.pdf"));
    EXPECT_EQ(mock.lastSubmitTitle, QStringLiteral("doc.pdf"));
    EXPECT_EQ(mock.lastSubmitSettings.copies, 3);
    EXPECT_EQ(mock.lastSubmitSettings.duplex, DuplexMode::LongEdge);
    EXPECT_EQ(mock.lastSubmitSettings.colorMode, ColorMode::Gray);

    mock.submitJobResult = false;
    EXPECT_FALSE(mock.submitJob(QStringLiteral("/tmp/other.pdf"),
                                QStringLiteral("other.pdf"), settings));
    EXPECT_EQ(mock.submitJobCallCount, 2);
}
